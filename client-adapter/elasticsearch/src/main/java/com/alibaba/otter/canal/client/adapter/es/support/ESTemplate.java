...
public class ESTemplate {
    ...
    public ESTemplate(RestHighLevelClient elasticSearchClient){
...
//        if(pkVal == null){
//            logger.warn("-------------------------insert id empty-------------------" + esFieldData);
//            return;
//        }
//        if(pkVal instanceof String && StringUtils.isBlank((String)pkVal)){
//            logger.warn("-------------------------insert id empty-------------------" + esFieldData);
//            return;
//        }
        if (StringUtils.isNotBlank(mapping.get_id())) {
            String parentVal = (String) esFieldData.remove("$parent_routing");
            if (mapping.isUpsert()) {
                UpdateRequest updateRequest = new UpdateRequest(mapping.get_index(),mapping.get_type(),pkVal.toString()).doc(esFieldData).docAsUpsert(true);
                if (StringUtils.isNotEmpty(parentVal)) {
                    updateRequest.routing(parentVal);
                }
                getBulk().add(updateRequest);
            } else {
                IndexRequest indexRequest = new IndexRequest(mapping.get_index(),mapping.get_type(),pkVal.toString()).source(esFieldData);
                if (StringUtils.isNotEmpty(parentVal)) {
            ...
            searchSource.query(QueryBuilders.termQuery(mapping.getPk(), pkVal));
            ...
            try {
                ...
                for (SearchHit hit : response.getHits()) {
                    getBulk().add(new UpdateRequest(mapping.get_index(),mapping.get_type(),hit.getId()).doc(esFieldData));
                    commitBulk();
                }
            }catch (IOException e){
...
//        if(pkVal == null){
//            logger.warn("-------------------------delete id empty-------------------" + esFieldData);
//            return;
//        }
//        if(pkVal instanceof String && StringUtils.isBlank((String)pkVal)){
//            logger.warn("-------------------------delete id empty-------------------" + esFieldData);
...
//        }
        if (StringUtils.isNotBlank(mapping.get_id())) {
            getBulk().add(new DeleteRequest(mapping.get_index(), mapping.get_type(), pkVal.toString()));
            commitBulk();
        } else {
            SearchSourceBuilder searchSource = new SearchSourceBuilder();
            searchSource.query(QueryBuilders.termQuery(mapping.getPk(), pkVal));
    ...
    public void commit() {
        if (getBulk().numberOfActions() > 0) {
            try {
                BulkResponse response = elasticSearchClient.bulk(getBulk(),RequestOptions.DEFAULT);
...
//        if(pkVal == null){
//            logger.warn("-------------------------append4Update id empty-------------------" + esFieldData);
//            return;
//        }
//        if(pkVal instanceof String && StringUtils.isBlank((String)pkVal)){
//            logger.warn("-------------------------append4Update id empty-------------------" + esFieldData);
...
//        }
        if (StringUtils.isNotBlank(mapping.get_id())) {
            ...
            if (mapping.isUpsert()) {
                ...
                UpdateRequest updateRequest = new UpdateRequest(mapping.get_index(),mapping.get_type(),pkVal.toString()).doc(esFieldData);
                if (StringUtils.isNotEmpty(parentVal)) {
                    updateRequest.routing(parentVal);
                }
                getBulk().add(updateRequest);
    ...
    private String getEsType(ESMapping mapping, String fieldName) {
        ...
        if (fieldType == null) {
            ...
            try {
                ...
                mappings = getMappingResponse.mappings();
            }catch (IOException e){
